...
class SignalLineTemplateDefaultsTests(SimpleTestCase):
    ...
    def test_anti_drop_copy_is_rendered_in_forms_and_help_page(self):
        ...
        self.assertIn("rhd_ok_reactivation_mode", scenario_content)
        self.assertIn("rhd_ok_rebound_threshold", scenario_content)
        self.assertIn("rhd_ok_reentry_max_drawdown", scenario_content)
        self.assertIn("100 €", scenario_content)
        game_content = self._template("game_scenario_form.html")
        self.assertIn("Signal anti-chute RHD — Repli depuis haut récent", game_content)
        self.assertIn("rhd_ok_reactivation_mode", game_content)
        self.assertIn("rhd_ok_rebound_threshold", game_content)
        ...
        self.assertIn("Mode classique", help_content)
        self.assertIn("Mode rebond confirmé", help_content)
        self.assertIn("65 €", help_content)
        self.assertNotIn("RHD_OK_RECOVERY", scenario_content)
        self.assertNotIn("RHD_OK_RECOVERY", game_content)
        self.assertNotIn("RHD_OK_RECOVERY", help_content)
