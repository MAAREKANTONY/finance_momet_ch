...
class WorkerStartupRecoveryTests(TestCase):
    def test_recover_active_jobs_after_worker_restart_fails_running_job_immediately(self):
        from core.job_recovery import recover_active_jobs_after_worker_restart

        bt = Backtest.objects.create(name="BT worker restart", scenario=Scenario.objects.create(name="Scenario worker restart"))
        bt.status = Backtest.Status.RUNNING
        bt.save(update_fields=["status"])
        job = ProcessingJob.objects.create(
            job_type=ProcessingJob.JobType.RUN_BACKTEST,
        ...
        stats = recover_active_jobs_after_worker_restart(worker_hostname="celery@test")

        job.refresh_from_db()
        bt.refresh_from_db()
        self.assertEqual(stats.failed, 1)
        self.assertEqual(job.status, ProcessingJob.Status.FAILED)
        self.assertIn("worker restart", job.error.lower())
        self.assertEqual(bt.status, Backtest.Status.FAILED)

    def test_worker_ready_signal_recovers_orphan_jobs(self):
        from stockalert.celery import recover_orphan_jobs_after_worker_ready
        ...
        recover_orphan_jobs_after_worker_ready(sender=SimpleNamespace(hostname="celery@test"))

        job.refresh_from_db()
        self.assertEqual(job.status, ProcessingJob.Status.FAILED)
        self.assertIn("orphan active job", job.error.lower())
