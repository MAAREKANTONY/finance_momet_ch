...
from .templatetags.backtest_extras import (
    ...
    line_gm_sell_market_exit_display,
    line_market_conditions_display,
)
...
def _scenario_form_context(*, form, mode: str, has_other_default: bool, scenario=None):
    universes = list(Universe.objects.filter(active=True).order_by("name"))
    exchange_qs = Symbol.objects.filter(active=True).exclude(exchange__isnull=True).exclude(exchange="").values_list("exchange", flat=True).distinct().order_by("exchange")
    sector_qs = Symbol.objects.filter(active=True).exclude(sector__isnull=True).exclude(sector="").values_list("sector", flat=True).distinct().order_by("sector")
    return {
        "form": form,
        "mode": mode,
        "scenario": scenario,
        ...
        "universes": universes,
        "exchanges": list(exchange_qs),
        "sectors": list(sector_qs),
    }
...
def scenario_create(request):
    ...
    if request.method == "POST":
        ...
        form = ScenarioForm()
    return render(request, "scenario_form.html", _scenario_form_context(form=form, mode="create", has_other_default=has_other_default))
...
def scenario_edit(request, pk: int):
    ...
    return render(request, "scenario_form.html", _scenario_form_context(form=form, mode="edit", has_other_default=has_other_default, scenario=scenario))
...
def scenario_duplicate(request, pk: int):
    ...
    context = _scenario_form_context(form=form, mode="duplicate", has_other_default=has_other_default)
    context["source_scenario"] = source
...
def _ticker_from_universe_snapshot_entry(entry) -> str:
...
def _backtest_snapshot_tickers(bt: Backtest, tickers_map: dict) -> list[str]:
    ...
    for entry in bt.universe_snapshot or []:
        _add(_ticker_from_universe_snapshot_entry(entry))
    if not tickers:
        for ticker in (tickers_map or {}).keys():
            _add(ticker)
    if not tickers and getattr(bt.scenario, "universe_mode", Scenario.UniverseMode.STATIC_TICKERS) == Scenario.UniverseMode.STATIC_TICKERS:
        for ticker in bt.scenario.symbols.order_by("ticker").values_list("ticker", flat=True):
            _add(ticker)
    return tickers


def _line_played_with_optional_daily(line: dict) -> bool:
    played = _line_has_executed_trade(line)
    if played:
        return True
    try:
        from .services.backtesting.results_offload import load_daily_from_line
        raw_daily = load_daily_from_line(line or {})
    except Exception:
        raw_daily = (line or {}).get("daily") or []
...
def backtest_results(request, pk: int):
    ...
    debug_all_tickers = request.GET.get("debug_all_tickers") == "1"
    ...
    for tk, te in tickers_map.items():
        for l in (te.get("lines") or []):
            ...
            if played and tk not in ticker_options_seen:
                ...
                played_options_by_ticker[tk] = option
                ticker_options_seen.add(tk)
    if debug_all_tickers:
        all_ticker_options = []
        for tk in _backtest_snapshot_tickers(bt, tickers_map):
            if tk in played_options_by_ticker:
                all_ticker_options.append(played_options_by_ticker[tk])
                continue
            lines = ((tickers_map.get(tk) or {}).get("lines") or [])
            line_for_option = lines[0] if lines else {}
            try:
                li = int((line_for_option or {}).get("line_index") or 1)
            except Exception:
                li = 1
        ...
        ticker_options = all_ticker_options
    ...
    if requested_option:
        ticker = requested_option["ticker"]
    elif requested_ticker in tickers_map:
    ...
    for tk, te in tickers_map.items():
        for l in (te.get("lines") or []):
            ...
            global_line_rows.append({
                ...
                "PNL_AMOUNT": final_row.get("PNL_AMOUNT"),
                "AVG_TRADE_AMOUNT": final_row.get("AVG_TRADE_AMOUNT"),
                "PROFIT_FACTOR_AMOUNT": final_row.get("PROFIT_FACTOR_AMOUNT"),
                "WIN_RATE_AMOUNT": final_row.get("WIN_RATE_AMOUNT"),
    ...
    return render(
        ...
        "backtest_results.html",
        {
            ...
            "debug_all_tickers": debug_all_tickers,
            "selected_ticker_option": selected_ticker_option,
...
def _build_backtest_workbook_full(bt):
    ...
    for ticker in selected_tickers_for_details:
        ...
        for line in (tentry or {}).get("lines") or []:
            ...
            append_excel_row(ws_s, [
                ...
                line_gm_push_buy_display(line),
                line_gm_sell_market_exit_display(line),
                line_gm_push_sell_market_exit_display(line),
                line.get("sell"),
    ...
    append_excel_row(ws_pd, ["Date", "Equity", "Invested", "GlobalCash", "CashAllocated", "PositionsValue", "PnL global", "Performance portefeuille (%)", "Moyenne globale rendements bornés Nglobal (%)", "Drawdown (%)"])
...
def _build_backtest_workbook_compact(bt, *, charts: str = "1", chart_mode: str = "top", chart_limit: str = "", chart_ticker: str = "", chart_line: str = ""):
    ...
    ws_s = wb.create_sheet("Summary")
    append_excel_row(ws_s, [
        "Ticker", "Line #", "BUY", "Conditions de marché", "GM Push BUY", "Protection marché GM", "Protection GM Push", "SELL", "Allocated",
    ...
    for ticker, tentry in tickers_map.items():
        ...
        for strat in lines:
            ...
            append_excel_row(ws_s, [
                ...
                line_gm_push_buy_display(strat),
                line_gm_sell_market_exit_display(strat),
                line_gm_push_sell_market_exit_display(strat),
                strat.get("sell"),
            ...
            try:
                ...
                # --- DYNAMIC UNIVERSE actions ---
                    ...
                    if not request.user.is_staff:
                        ...
                        launch = launch_processing_job(
                            task=map_universe_membership_symbols_job_task,
                            job_type=ProcessingJob.JobType.FETCH_BARS,
                            created_by=request.user if request.user.is_authenticated else None,
                            message=f"Mapping des actions {universe_code} depuis memberships CSV en attente ({mode_label})",
                            task_kwargs={
                                "universe_code": universe_code,
                        ...
                        if launch.dispatch_error:
                            raise launch.dispatch_error
                        messages.success(request, f"Mapping des actions {universe_code} lancé en arrière-plan ({mode_label}, job #{launch.job.id}).")
